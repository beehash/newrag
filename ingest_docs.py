#!/usr/bin/env python3
"""
将 docs 目录中的所有文件入库到 milvus 数据库的 zw_docs 集合中
"""
...
def process_file(file_path, embedding_service, mv_store):
    """
    处理单个文件并入库
...
def main():
    ...
    for file_path in files_to_process:
        if process_file(file_path, embedding_service, mv_store):
            success_count += 1
        else:
            failure_count += 1
    ...
    print(f"总处理: {success_count + failure_count} 个文件")
    
    # 关闭连接
    print("\n关闭数据库连接...")
    # 注意：MVStore类目前没有提供close方法，这里可以添加
    print("脚本执行完成！")
